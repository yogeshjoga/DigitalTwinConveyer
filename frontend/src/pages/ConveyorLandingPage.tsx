...
export default function ConveyorLandingPage() {
    ...
    <div style={{ fontFamily: "Inter, sans-serif", backgroundColor: "#0a0f1e", color: "#f1f5f9", minHeight: "100vh" }}>
      <nav style={{ position: "sticky", top: 0, zIndex: 50, backgroundColor: "rgba(10,15,30,0.92)", backdropFilter: "blur(12px)", borderBottom: "1px solid rgba(255,255,255,0.08)", padding: "0 2rem", height: 64, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", gap: 12 }}>
          <button onClick={() => navigate("/landing")} style={{ padding: "8px 20px", borderRadius: 10, border: "1px solid rgba(255,255,255,0.15)", background: "transparent", color: "#94a3b8", fontSize: 13, cursor: "pointer" }}>Back to Ecosystem</button>
          <button onClick={() => navigate("/")} style={{ padding: "8px 20px", borderRadius: 10, border: "1px solid rgba(255,255,255,0.15)", background: "transparent", color: "#94a3b8", fontSize: 13, cursor: "pointer" }}>Fleet Overview</button>
        ...
        <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4, duration: 0.7 }} style={{ marginTop: 80, display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 16, maxWidth: 800, width: "100%" }}>
          {[["44+","Belt Types Monitored"],["2s","Sensor Refresh Rate"],["6","Sensor Channels"],["10min","Auto-Stop Gate"]].map(([v,l]) => (
            <div key={l} style={{ padding: "20px 16px", borderRadius: 16, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", textAlign: "center" }}>
              ...
              <div style={{ fontSize: 11, color: "#64748b", margin: "4px 0 0 0" }}>{l}</div>
            </div>
          ))}
        </motion.div>
      </section>
      ...
      <section style={{ padding: "6rem 2rem", background: "radial-gradient(ellipse 80% 60% at 50% 100%, rgba(39,163,114,0.15) 0%, transparent 70%)" }}>
      ...
      <section style={{ padding: "6rem 2rem", maxWidth: 1200, margin: "0 auto" }}>
        ...
        <div style={{ padding: 24, borderRadius: 16, background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", fontFamily: "monospace", fontSize: 12, color: "#64748b", lineHeight: 2, textAlign: "center" }}>
